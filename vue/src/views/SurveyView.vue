    ...
    <PageComponent>
        <template v-slot:header>
            <div class="flex items-center justify-between">
                ...
                <button v-if="route.params.id" type="button" @click="deleteSurvey()" class="py-2 px-3 text-white bg-red-500 rounded-md hover:bg-red-400">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 -mt-1 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<pre>
    ...
    <form v-else @submit.prevent="saveSurvey">
        ...
        <!-- Image -->
            <div>
                ...
                <div class="mt-1 flex items-center">
                    ...
                    <button type="button" class="relative overflow-hidden ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray50
                      focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"> <input type="file" @change="onImageChoose" class="absolute left-0 top-o right-0 bottom-0 opacity-0 cursor-pointer" /> Change </button>
                </div>
            </div>
        <!-- /Image -->
        ...
        <!-- Description -->
         ...
         <div class="mt-1">
             <textarea name="description" id="description" rows="3" v-model="model.description" autocomplete="survey_description" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Describe your survey" />
...
// Watch to current survey data change when this happens we update Local model variable
watch(
...
function onImageChoose (ev) {
    ...
    reader.onload = () => {
        ...
        // The field to display the image here
